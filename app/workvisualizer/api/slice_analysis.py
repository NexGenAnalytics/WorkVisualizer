...
def process_file(filepath, repr_slice_stats, num_slices, slices):
    ...
    for slice_id in list(slices.keys()):

        # First, calculate the time lost in Allreduce functions
        time_lost = slice_stats[slice_id]["type times"]["MPI_Allreduce"] - repr_slice_stats[slice_id]["type times"]["MPI_Allreduce"]
        ...
        for call_type, count in slice_stats[slice_id]["type counts"].items():
            type_counts_pct_diffs[call_type] = (count / repr_slice_stats[slice_id]["type counts"][call_type]) - 1
        ...
        for call_type, time in slice_stats[slice_id]["type times"].items():
            type_times_pct_diffs[call_type] = (time / repr_slice_stats[slice_id]["type times"][call_type]) - 1
...
def run_slice_analysis(files_dir, representative_rank, representative_slices):
    ...
    time_losing_rank_slices = find_rank_slices_with_most_time_lost(all_slices, num_entries=10)
    print(time_losing_rank_slices)
...
def main():
    ...
    representative_slices = {0: (0.0, 0.21), 1: (0.21, 0.41), 2: (0.41, 0.64), 3: (0.64, 0.86), 4: (0.86, 1.11), 5: (1.35, 1.5)}
